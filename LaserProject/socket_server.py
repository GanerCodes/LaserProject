...
class  Socket_Server:
    ...
    def  handle_client_message( SPECIAL_SELF_ , C, T): # todo
        SPECIAL_SELF_. transmit(T)
    
    def  transmit( SPECIAL_SELF_ , data, target= None ):
        print * SPECIAL_SELF_. Clients
        B = <$x$> SPECIAL_SELF_. Sock_B.sendto(data.encode(), (x, SPECIAL_SELF_. PORT_B))
        B(target) if target else  DEGEN_( B(c) for c in  SPECIAL_SELF_. Clients.values())
    
    def  __call__( SPECIAL_SELF_ ):
        while  True :
            data, (ip, port) = SPECIAL_SELF_. Sock_R.recvfrom(1024)
            C, T = data.decode().split(":",1)
            if C not in  SPECIAL_SELF_. Clients:
                SPECIAL_SELF_. Clients[C] = ip
                print *f"""Added client {C} - {ip}""" 
            elif (p:= SPECIAL_SELF_. Clients[C])!=ip:
                SPECIAL_SELF_. Clients[C] = ip
                print *f"""Changed {C}'s IP: {p}>>{ip}""" 
            print *f"""{C}:{T}""" 
            SPECIAL_SELF_. handle_client_message(C, T)
